...
it('computes PTR, PTS, scheme, GST and profit correctly', function () {
    ...
    expect(round($r['ptr'], 2))->toBe(96.0);
    expect(round($r['pts'], 2))->toBe(86.4);
    expect(round($r['effective_pts'], 2))->toBe(66.46);
    expect(round($r['retailer_margin_per_unit'], 2))->toBe(24.0);
    ...
    // Cross-check: per-unit invoice total equals PTS.
    expect(round($r['taxable_value_pts'] + $r['gst_amount_pts'], 2))->toBe(86.4);
    expect(round($r['profit_per_unit'], 2))->toBe(58.46);
    expect(round($r['markup_percent'], 1))->toBe(730.8);
